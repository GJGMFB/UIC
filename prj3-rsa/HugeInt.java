...
public class HugeInt {
    ...
    /**
     * Computes b^e mod m. e must be positive. This is the lazy implementation.
     *
     * @param  b An integer
     * @param  e An integer
     * @param  m An integer
     ...
     */
    public static HugeInt modPow(HugeInt b, HugeInt e, HugeInt m) {
        if (equalsHugeInt(m, new HugeInt("1"))) {
            return new HugeInt("0");
        }

        HugeInt c = new HugeInt("1");
        HugeInt i = new HugeInt("0");
        while (HugeInt.lessThan(i, e)) { // i < e
            c = modHugeInt(multiplyHugeInt(c, b), m); // c*b mod m
            i = addHugeInt(i, new HugeInt("1")); // i++
        }
